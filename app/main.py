...
class ScreenCaptureApp:
    ...
    def show_screenshot_for_annotation(self, screenshot):
        ...
        # Save temporary screenshot for annotation
        temp_path = "temp_screenshot.png"
        screenshot.save(temp_path)

        # Initialize AnnotationTool
        annotation_tool = AnnotationTool(annotation_win, canvas, temp_path)

        # Bind events specifically for annotation purposes
        canvas.bind(
            "<ButtonPress-1>", lambda event: annotation_tool.on_button_press(event)
        )
        canvas.bind("<B1-Motion>", lambda event: annotation_tool.on_move_press(event))
        canvas.bind(
            "<ButtonRelease-1>",
            lambda event: annotation_tool.on_annotation_release(event, self.master),
        )
